...
#include <string.h>
#include <ctype.h>
void utnMostrarValoresVariosTipos(float numeracion[], char cadenaCaracteres[][50], int numeroA[], int numeroB[],
		int cantidad)
{
...
int utnCargaValoresVariosTipos(float numeracion[], char cadenaCaracteres[][50], int numeroA[], int numeroB[],
		int cantidad,char* mensajeNumeracion,char* mensajeNumeroA, char* mensajeNumeroB, char* mensajeCadenaChar)
{
	int retorno=0;
	for (int i = 0; i < cantidad; i++)
	{
		retorno=utnGetNumero(&numeracion[i], mensajeNumeracion, "Error", "Mensaje seguir o no", 0, 10, 2);
		printf("%d",retorno);
		if(retorno==-1)
		{
			printf("Error");
		...
		numeroA[i] = IngresarEntero(mensajeNumeroA);
		numeroB[i] = IngresarEntero(mensajeNumeroB);
